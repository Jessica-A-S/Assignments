def main():
    ...
    while options != "Q":
        if options == "I":
            ...
            while distance >= 1:
                run_play(distance, SHOT_NUM)
            if distance < 1:
                execute_game_ending_sequence(par, SHOT_NUM, TOTAL_SHOT_NUM, TOTAL_HOLES)
            options = input("Golf!\n(I)nstructions \n(P)lay round \n(Q)uit \n").upper()
    if options == "Q":
...
def run_play(distance, SHOT_NUM):
    ...
    if club_selection == "d":
        ...
        return run_play(distance, SHOT_NUM)
    return run_play(distance, SHOT_NUM)


def execute_game_ending_sequence(par, SHOT_NUM, TOTAL_SHOT_NUM, TOTAL_HOLES):
    if SHOT_NUM > par:
        shots_to_par = SHOT_NUM - par
        TOTAL_HOLES += 1
        TOTAL_SHOT_NUM += SHOT_NUM
        print("Clunk... After {} hits, the ball is in the hole! \nDisappointing. You are {} over par."
